...
{
    public class DamageWorker_Shardblade : DamageWorker_AddInjury
    {
        public override DamageResult Apply(DamageInfo dinfo, Thing thing)
        {
            // Code from base DamageWorker_AddInjury
            if (!(thing is Pawn pawn))
            {
                return base.Apply(dinfo, thing);
            }
            DamageResult result = ApplyToPawn(dinfo, pawn);
            // Check if application of damage killed the pawn
            if (pawn.Dead) 
            {
                Log.Message("Rimlight Archive: Pawn killed by Shardblade");
                // Get the pawns eyes in an enumerable 
                ...
                {
                    Log.Message($"Removing {part.Label}");
                    if (part.def == BodyPartDefOf.Eye)
                    {
                        // Create a MissingBodyPart HediffDef, make a Hediff for the pawn and then apply it to their eye
                        HediffDef hediff = HediffDef.Named("MissingBodyPart"); 
                        Hediff removeEye = HediffMaker.MakeHediff(hediff, pawn);
                        pawn.health.AddHediff(removeEye, part);
                    }
                }
